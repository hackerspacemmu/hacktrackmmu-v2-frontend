...
test.describe("Onboarding", () => {
  ...
  test.beforeEach(async () => {
    // Fixture is created directly at "contacted" -- the scenario's starting status --
    // rather than clicked through the registered stage to get there.
    member = await createMember({
      name: uniqueMemberName("Onb Demote", test.info().workerIndex),
      status: "contacted",
    });
  });
  ...
  test("Demoting onboarding status reverts to the previous stage", async ({
    page,
  }) => {
    // 1. Navigate to /login, log in as admin, go to /onboarding, search for the fixture
    // member (status 'Contacted') and open its View modal
    ...
    // expect: The modal's Status heading reads `Status: Contacted`
    await expect(
      modal.getByRole("heading", { name: "Status: Contacted" }),
    ).toBeVisible();

    // expect: An enabled (yellow) down-arrow Demote button is visible (its title is
    // `Demote to Registered`). When demotion is not possible this renders as a
    // non-interactive grey <div> instead, so getByRole("button", ...) only matches the
    // enabled, interactive variant.
    const demoteButton = modal.getByRole("button", {
      name: "Demote to Registered",
    });
    ...
    // expect: The Status heading updates to `Status: Registered`
    await expect(
